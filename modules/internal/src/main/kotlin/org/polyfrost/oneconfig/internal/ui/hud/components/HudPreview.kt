...
internal class HudPreviewState(val runtime: PolyComposeRuntime) {
    ...
    var drawRevision by mutableIntStateOf(0)
        private set

    private val drawReadObserver = SnapshotStateObserver { it() }.also { it.start() }
    private val onDrawStateChanged: (Any) -> Unit = { invalidateDraw() }
    ...
    fun render(context: RenderContext) {
        Snapshot.withoutReadObservation {
            drawReadObserver.observeReads(this, onDrawStateChanged) {
                runtime.root.render(context)
            }
        }
    }

    fun dispose() {
        drawReadObserver.clear()
        drawReadObserver.stop()
        runtime.dispose()
    }
}
...
internal fun rememberHudPreview(hud: Hud): HudPreviewState {
    ...
    val alwaysRedraw = hud.alwaysRedraw
    ...
    LaunchedEffect(state, alwaysRedraw) {
        if (!alwaysRedraw) return@LaunchedEffect
        while (true) {
            withFrameNanos { state.invalidateDraw() }
        }
    }
    return state
}
...
internal fun HudPreviewCanvas(state: HudPreviewState, scale: Float, modifier: Modifier = Modifier) {
    Canvas(modifier) {
        ...
        drawIntoCanvas { canvas ->
            ...
            state.render(RenderContext(skia))
            skia.restore()
        }
    }
}
